...
def buttons(message):
    ...
    markup_reply.add(item_invest, item_profile, item_factory, item_take, item_vivod, item_tgchannel)
...
def get_text(message):
  ...
  # если "аргумент" цифра
    ...
    if message.text == 'Получить бонус':
        ...
        client.send_message(message.chat.id, f'Подпишитесь на наш Telegram канал, чтобы получить бонус, (при отписке, бонус обнуляется!): {url}')
        ...
        if not is_subscribed(chat_id, user_id):
            client.send_message(chat_id, 'Вы не подписаны на канал!')
        else:
            db = sqlite3.connect('allusbase.db', check_same_thread = False)
            sql = db.cursor()
            for i in sql.execute(f"SELECT cash FROM users WHERE id = {user_id}"):
                balance = i[0]
                break
            sql.execute(f"UPDATE users SET cash = {0.5 + balance} WHERE id = {user_id}")
            db.commit()
            client.send_message(message.chat.id, '✅Вы успешно получили 0.5 рублей за подписку!')
