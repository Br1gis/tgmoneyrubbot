            ...
            while True:
                ...
                if api.check(comment):
                    ...
                    for i in sql.execute(f"SELECT cash_energy FROM users WHERE id = {user_id}"):
                        ...
                        db.commit()
                        client.edit_message_text(chat_id = message.chat.id, message_id = message.message_id, text =  f'*✅Вы успешно инвестировали {price} руб!*', parse_mode = "Markdown")
                        all_deps += price
