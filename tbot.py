...
def registration(message):
    if message.text == 'пропуск':
        ...
        if sql.fetchone() is None:
            user_id = message.from_user.id
            name = message.from_user.first_name
            sql.execute(f"INSERT INTO users VALUES (?, ?, ?, ?, ?)", (name, user_id, 0, 0.00, 0))
            db.commit()
...
def buttons(message):
    ...
    markup_reply.add( item_profile, item_factory, item_bonus, item_referals, item_techhelp, item_botssale, item_otzchannel, item_info)
