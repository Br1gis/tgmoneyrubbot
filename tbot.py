                ...
                if api.check(comment):
                    ...
                    for i in sql.execute(f"SELECT cash_energy FROM users WHERE id = {user_id}"):
                        ...
                        if username is None:
                            ...
                            client.send_message(-1001638482952, f'[Пользователь](https://t.me/{username}) пополнил {priceы}₽*\nПлатёжная система: QIWI', parse_mode = "Markdown")
...
def buttons(message):
    ...
    markup_reply.add( item_profile, item_factory, item_bonus, item_techhelp, item_botssale, item_otzchannel, item_info)
...
def get_text(message):
  ...
  # если "аргумент" цифра
    ...
    if message.text == '📕О боте':
        ...
        markup_inlines.add(item_owner, item_userlogs)
...
def factory(message):
    ...
    for i in sql.execute(f"SELECT cash_energy FROM users WHERE id = '{user_id}'"):
        ...
        client.send_message(message.chat.id, f'*💸Всего инвестировано:* {all_deps}₽\n\n*💸Доход в день:*{all_vivods}₽', reply_markup = button_reply, parse_mode = "Markdown")
