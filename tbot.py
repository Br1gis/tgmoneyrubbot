...
def buttons(message):
    ...
    item_tgchannel = types.KeyboardButton(text = 'Получить бонус')
    item_techhelp = types.KeyboardButton(text = 'Задать вопрос')
    item_botssale = types.KeyboardButton(text = 'Хочу такого же бота!')

    markup_reply.add(item_invest, item_profile, item_factory, item_take, item_vivod, item_tgchannel, item_techhelp, item_botssale)
...
def get_text(message):
  ...
  # если "аргумент" цифра
    ...
    if message.text == 'Получить бонус':
        user_id = message.from_user.id
        if not user_id in got_bonus:
            chat_id = -1001616425231
            url = 'https://t.me/rubmoneychannel'
            client.send_message(message.chat.id, f'Подпишись на наш телеграм канал с новостями и промокодами и получи бонус! {url}')
            def is_subscribed(chat_id, user_id):
                try:
                    client.get_chat_member(chat_id, user_id)
                    return True
            ...
            if not is_subscribed(chat_id, user_id):
                client.send_message(chat_id, 'Вы не подписаны!')
            else:
                db = sqlite3.connect('allusbase.db', check_same_thread = False)
                sql = db.cursor()
                for i in sql.execute(f"SELECT cash FROM users WHERE id = {user_id}"):
                    balance = i[0]
                    break
                sql.execute(f"UPDATE users SET cash = {0.3 + balance} WHERE id = {user_id}")
                db.commit()
                client.send_message(message.chat.id, '✅Вы получили 0.3 руб. за подписку!')
                got_bonus.append(user_id)
        else:
            client.send_message(message.chat.id, 'Вы уже получали бонус!')
