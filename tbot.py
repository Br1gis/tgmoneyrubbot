...
def glasses(message):
    ...
    print(stavka)
    for i in sql.execute(f"SELECT cash FROM users WHERE id = {message.chat.id}"):
        balance = i[0]
        if stavka < balance:
            global botnum
            botnum = randint(1, 3) 
            markup_inline = types.InlineKeyboardMarkup(row_width = 3)
            glass1 = types.InlineKeyboardButton(text = '🥛', callback_data = '1')
            glass2 = types.InlineKeyboardButton(text = '🥛', callback_data = '2')
            glass3 = types.InlineKeyboardButton(text = '🥛', callback_data = '3')
            markup_inline.add(glass1, glass2, glass3)
            client.send_message(message.chat.id, 'Ставки сделаны! Выбирайте стакан:', reply_markup = markup_inline)
        else:
            client.send_message(message.chat.id, '⛔️ У вас недостаточно средств!')
...
def endsbor(message):
    if message.text == 'Инвест':
        for i in sql.execute(f"SELECT cash_energy FROM users WHERE id = {sborid}"):
            balance = i[0]
            sql.execute(f"UPDATE users SET cash_energy = {balance - sborsum} WHERE id = {sborid}")
            db.commit()
            client.send_message(message.chat.id, '✅ Успешно!')
            client.send_message(int(sborid), f'*Администратор забрал у вас {givesum}₽ с баланса для инвестиций!*', parse_mode = 'Markdown')
            break
    if message.text == 'Вывод':
...
def vivodmoney(message):
    ...
    if message.text == '-':
        ...
        db = sqlite3.connect('allusbase.db', check_same_thread = False)
        sql = db.cursor()
        for t in sql.execute(f"SELECT cash_energy FROM users WHERE id = {user_id}"):
            global balrub
            balrub = t[0]
            break
        if balrub > 0:
            ...
            if balance >= 2:
                ...
                client.send_message(984674439, f'Заявка на вывод средств:\n\n*Имя:* {message.from_user.first_name}\n*ID:* {user_id}\n*QIWI:* {pricev}\n*Сумма вывода:* {round(commisia, 2)}', reply_markup = markup_inline, parse_mode = "Markdown")
                

            else:
                client.send_message(message.chat.id, '⛔️ Недостаточно средств! Мин. сумма 2 руб.')
        else:
            client.send_message(message.chat.id, 'Ошибка! Вы еще не инвестировали денег для вывода!')
...
def get_text(message):
    ...
    if message.text == '👨🏻‍💻Мой профиль':
        ...
        for i in sql.execute(f'SELECT cash FROM users WHERE id = {user_id}'):
            for ab in sql.execute(f'SELECT cash_energy FROM users WHERE id = {user_id}'):
                for c in sql.execute(f'SELECT vivods FROM users WHERE id = {user_id}'):
                    client.send_message(message.chat.id, f'*Ваше имя:* {name}\n\n*ID:* {user_id}\n\n*Ваш баланс:* {round(i[0], 2)}₽\n\n*💸Всего пополнено:* {ab[0]}₽\n🤑*Всего выведено:* {c[0]}₽', reply_markup = markup_inline, parse_mode = "Markdown")
                    break
    ...
    if message.text == '🎲Игры':
        ...
        item_glasses = types.KeyboardButton(text = '🥛Стаканчики')
        item_back = types.KeyboardButton(text = '🚫Назад')
        markup_plays.add(item_luckybox, item_glasses, item_back)
    ...
    if message.text == '🥛Стаканчики':
        msg = client.send_message(message.chat.id, 'Пожалуйста, напишите вашу ставку')
        client.register_next_step_handler(msg, glasses)
...
def callba(call):
    ...
    if call.data == 'pay':
        ...
        wallet = pyqiwi.Wallet(token=token, number=phone)
        payment = wallet.send(pid=99, recipient=f'{pricev}', amount=round(commisia, 2), comment='Выплата от: RubMoneyBot 💸')
        for r in sql.execute(f'SELECT vivods FROM users WHERE id = {call.message.chat.id}'):
            balviv = r[0]
            sql.execute(f'UPDATE users SET cash = {balviv + commisia} WHERE id = {call.message.chat.id}')
            db.commit()
    ...
    if call.data == 'paybox':
        ...
        for u in sql.execute(f'SELECT cash FROM users WHERE id = {call.message.chat.id}'):
            if u[0] > 5:
                sql.execute(f'UPDATE users SET cash = {u[0] + prize - 5} WHERE id = {call.message.chat.id}')
                db.commit()
    ...
    if call.data == '1':
        ...
        if botnum == usernum:
            for u in sql.execute(f'SELECT cash FROM users WHERE id = {call.message.chat.id}'):
                prise = stavka * 2
                sql.execute(f'UPDATE users SET cash = {u[0] + prise - stavka} WHERE id = {call.message.chat.id}')
                db.commit()
                client.edit_message_text(chat_id = call.message.chat.id, message_id = call.message.message_id, text =  f'*Поздравляем! Вы отгадали стаканчик и получили {prise}₽*', parse_mode = "Markdown")
                time.sleep(1)
                buttons(call.message)
                break
        else:
            for u in sql.execute(f'SELECT cash FROM users WHERE id = {call.message.chat.id}'):
                prise = stavka * 2
                sql.execute(f'UPDATE users SET cash = {u[0] - stavka} WHERE id = {call.message.chat.id}')
                db.commit()
                client.edit_message_text(chat_id = call.message.chat.id, message_id = call.message.message_id, text =  f'*К сожалению, вы не угадали :(*', parse_mode = "Markdown")
                time.sleep(1)
                buttons(call.message)
                break
    if call.data == '2':
