...
def endgive(message):
    ...
    if message.text == 'Инвест':
        for i in sql.execute(f"SELECT cash_energy FROM users WHERE id = {gid}"):
            balance = i[0]
            sql.execute(f"UPDATE users SET cash_energy = {int(gsum) + balance} WHERE id = {gid}")
            db.commit()
            client.send_message(message.chat.id, '✅ Успешно!')
            client.send_message(int(gid), f'*Администратор начислил вам {givsum}₽ на баланс для инвестиций!*', parse_mode = 'Markdown')
            break
    if message.text == 'Вывод':
        for i in sql.execute(f"SELECT cash FROM users WHERE id = {gid}"):
            balance = i[0]
            sql.execute(f"UPDATE users SET cash = {gsum + balance} WHERE id = {gid}")
            db.commit()
            client.send_message(message.chat.id, '✅ Успешно!')
            client.send_message(int(gid), f'*Администратор начислил вам {gsum}₽ на баланс для вывода!*', parse_mode = 'Markdown')
            break
...
def endsbor(message):
    if message.text == 'Инвест':
        for i in sql.execute(f"SELECT cash_energy FROM users WHERE id = {giveid}"):
            balance = i[0]
            sql.execute(f"UPDATE users SET cash_energy = {givesum - balance} WHERE id = {giveid}")
            db.commit()
            client.send_message(message.chat.id, '✅ Успешно!')
            client.send_message(int(giveid), f'*Администратор забрал у вас {givesum}₽ с баланса для инвестиций!*', parse_mode = 'Markdown')
            break
    elif message.text == 'Вывод':
        for i in sql.execute(f"SELECT cash FROM users WHERE id = {giveid}"):
            balance = i[0]
            sql.execute(f"UPDATE users SET cash = {givesum - balance} WHERE id = {giveid}")
            db.commit()
            client.send_message(message.chat.id, '✅ Успешно!')
            client.send_message(int(giveid), f'*Администратор забрал у вас {givesum}₽ с баланса для вывода!*', parse_mode = 'Markdown')
            break
        ...
        def callba(call):
            ...
            if call.data == 'paybox':
                ...
                for u in sql.execute(f'SELECT cash FROM users WHERE id = {message.chat.id}'):
                    sql.execute(f'UPDATE users SET cash = {5 - u[0]}')
                    db.commit()
